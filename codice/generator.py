...
'''
def residual_block(inputs, activation='relu'):

	#First half
	nn = BatchNormalization()(inputs)
	nn = Activation(activation)(nn)
	nn = Dense(units=500)(nn)
	#Second half
...
def build_generator(input_shape, res_block_num=100, num_classes=64, attention_on=False, masking_on=True):
	...
	for index in range(res_block_num):
		...
		res_block_y = residual_block(inputs=nn, activation='relu')
	...
	adam = optimizers.Adam(lr=0.0006, beta_1=0.9, beta_2=0.999) #, decay=1e-3)

	#compile generator model
	model.compile(loss='categorical_crossentropy', optimizer=adam, metrics=['acc'])
	#print model structure to file
	with open('model_struct.txt', 'w') as model_file:
...
if __name__ == "__main__":
	...
	x_train, y_train, x_val, y_val, x_test, y_test = my_utils.load_generator_data(training_data_file = "DS_GENERATOR/DS.UNIQUES.B.txt", test_data_file="DS_GENERATOR/DS.UNIQUES.A.txt")
	...
	trained_model.save('generator_model_BA.h5')
	#check solutions feasibility for training, validation and test sets
	feas_ratios_train = my_utils.check_solutions_feasibility(trained_model, x_train, "DS.FULL.SOLUTIONS.txt")
	feas_ratios_val = my_utils.check_solutions_feasibility(trained_model, x_val, "DS.FULL.SOLUTIONS.txt")
